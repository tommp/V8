...
bool Texture::load_binary_image(const std::string& name, unsigned char* image_ptr){
	...
	unsigned char* image = (unsigned char*)std::malloc(sizeof(unsigned char) * width * height);
...
bool Texture::load_from_file(const std::string& name){
	...
	if (!load_binary_image(name, image)) {
