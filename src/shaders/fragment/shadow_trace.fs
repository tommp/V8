...
void main(){
	...
	float distance = length(light_positions[gl_InstanceID] - frag_position);
	vec3 light_direction = normalize(light_positions[gl_InstanceID] - frag_position);

	
	float shadow_occlusion = 1.0;
	float num_steps = distance / stepsizes[gl_InstanceID];
	float diff;

	vec3 trace_offset = light_direction * stepsizes[gl_InstanceID];
	vec3 trace_position = frag_position + trace_offset * loop_offsets[gl_InstanceID];
	vec3 sample_position = trace_position;

	for (float counter = loop_offsets[gl_InstanceID]; counter < num_steps - loop_offsets[gl_InstanceID]; counter += stepsizes[gl_InstanceID]) {
		...
		shadow_occlusion -= (1 - step(shadow_slacks[gl_InstanceID, diff));
	}
